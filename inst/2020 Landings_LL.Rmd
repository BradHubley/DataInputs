...
nafoB=get_21B(count="others",ystart,yend,1, datadir=datadir)
# unique(nafoB$Division)
nafoBdiv = get_21B(count="others",ystart,yend,2, datadir=datadir)
...
# current 21B with the version in the last assessment. getwd()
...
## Scal format of 21B  
nafoBg = get_21B(count="others",1970,2016,4,datadir=datadir)
...
nafoABMmiss= nafoABM[is.na(nafoABM$CatchB),]  %>%
  ...
  filter (Year<=yend)
write.csv(nafoABMmiss, file.path(wd,"data","Missing_21B.csv"))
...
nafoS= get_SCALformat(nafoland)
    ...
    with(nafoS, plot(Year,LL4,type='l',ylim=c(0,3000),ylab="Landings (t)"))
   with(nafoS,  lines(Year,LL3,col=2))
   with(nafoS,  lines(Year,OT4,lty=2))
   with(nafoS,  lines(Year,OT3,lty=2,col=2))
