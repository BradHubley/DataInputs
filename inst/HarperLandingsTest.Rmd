...
# annual TAC table
TAC = data.frame(Year=1988:2026,TAC=c(3200,3200,3200,3200,3200,3200,1500,850,850,850,850,850,1000,1150,1150,1300,1300,1375,1475,1475,1475,1700,1850,1850,2128,2447,2563,2738,3149,3621,4164,4789,5507,5445,4807, 4744, 4927, 5149, 5064)) ##update each year

# 21B start year
ystart =1960

# 21A last year(assessment year)
yend=2025 ##DOES THIS CHANGE??
...
Canada = merge(merge(data.frame(Year=1960:2025), nafoCDN3,all=T), nafoCDN4,all=T) ##CHANGE TO 2023???
Foreign  = merge(merge(data.frame(Year=1960:2025), nafoForeign3,all=T), nafoForeign4,all=T) ##CHANGE TO 2023???
...
Yearavg= rep(c("1960-1969","1970-1979","1980-1989","1990-1999","2000-2009","2010-2019"),each=10)
table11= subset(table1, Year<2020)
...
table12= subset(table1, Year>=2020) %>%
...
write.csv(table2,"C:/Users/harperd/Documents/Halibut/GitDataInputs/DataInputs_2023/DataInputs/datafolder/update_table1_2025.csv",row.names = F)
write.csv(table1,"C:/Users/harperd/Documents/Halibut/GitDataInputs/DataInputs_2023/DataInputs/datafolder/figuredata_2025.csv",row.names = F)
table2

table2<-read.csv("C:/Users/harperd/Documents/Halibut/GitDataInputs/DataInputs_2023/DataInputs/datafolder/update_table1_2025.csv")
...
cxn <- ROracle::dbConnect(DBI::dbDriver("Oracle"), "hubleyb", "R4#vmxtas", "PTRAN")

get_data(db='marfis', cxn = cxn)
...
PRO_SPC_INFO <- PRO_SPC_INFO %>% mutate(YEAR = as.numeric(substr(LANDED_DATE, 1,4)))
...
figtab[nrow(figtab) + 1, 1:3] = c(2025, "MARFIS", CanadaMar_ay$CanadaTotal)
write.csv(figtab, "C:/Users/harperd/Documents/Halibut/GitDataInputs/DataInputs_2023/DataInputs/FSAR/fsardata/figuredata_markdown_2025.csv", row.names = F)
...
write.csv(landingstable, file.path(wd,"FSAR/fsardata/landingstable.csv"), row.names = F)
...
plm = rbind(table1$CanadaTotal,table1$ForeignTotal,c(rep(NA,65),CanadaMar_ay$CanadaTotal)) ##64 becomes 65 for 2025
plm[is.na(plm)]<-0

png(file.path(wd,"figures","Fig2 fishery 2025_updated.png"), width = width1, height = height2,units='in',pointsize=12, res=300,type='cairo')
...
barplot(plm,ylim=c(0,5600), axes = FALSE, col=c('#377eb8','#d53e4f','#377eb8'),border=NA,density=c(NA,NA,40))
 ...
 axis(1,at=seq(0.7,78,1.2),lab=F,tck=-0.01) ##change to 78 in sequence to increase axis length (ticks)
 axis(1,at=seq(0.7,75,12),lab=seq(1960,ay,10))##75 remains (no change) since 2025 not a multiple of 10
 axis(2, at = seq(0, 5600, 1000), las = 1, 
     labels = prettyNum(seq(0, 5600,1000), 
                       big.mark = ',', 
                       scientific = FALSE))
 lines(seq(0.7,79.2,1.2),table1$TAC,col='#7fbc41',lwd=4) ##change to 78 to accommodate additional year (2025)
# grid(ny=NULL,nx=NA)
