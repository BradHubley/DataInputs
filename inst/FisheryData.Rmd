...
table1<- MARFISdata %>% filter(YEAR%in%yrs1&NAFO=='3PS') %>% group_by(QUARTER) %>% summarise(Catch_Kg = round(sum(RND_WEIGHT_KGS,na.rm=T)),Effort_Sets=sum(SET),CPUE=Catch_Kg/Effort_Sets)  %>% tibble()


table2<- MARFISdata %>% filter(YEAR%in%yrs2&NAFO=='3PS') %>% group_by(QUARTER) %>% summarise(Catch_Kg = round(sum(RND_WEIGHT_KGS,na.rm=T)),Effort_Sets=sum(SET),CPUE=Catch_Kg/Effort_Sets)  %>% tibble()

write.csv(table1,file.path(datadir,paste0("MARFIS_nafo3PS_quarter_",range(yrs1)[1],'-',range(yrs1)[2],".csv")),row.names=F)
write.csv(table2,file.path(datadir,paste0("MARFIS_nafo3PS_quarter_",range(yrs2)[1],'-',range(yrs2)[2],".csv")),row.names=F)
