...
MARFISdataLL$CLASS[MARFISdataLL$LOA>=35&MARFISdataLL$LOA<50]<-"LL35-50"
MARFISdataLL$CLASS[MARFISdataLL$LOA>=50&MARFISdataLL$LOA<65]<-"LL50-65"
...
tmp<- MARFISdata |>
  filter(YEAR%in%yrs&!is.na(NAFO)&CLASS%in%c("LL35","LL35-50")) |>
  group_by(NAFO,YEAR) |>
  summarise(Catch = sum(RND_WEIGHT_KGS,na.rm=T),Effort = sum(SET),CPUE =  Catch/Effort) 

#tmp<-aggregate(RND_WEIGHT_KGS~NAFO+YEAR,data=MARFISdata,FUN=sum,na.rm=T)
...
bioMap(ylim=c(42.5, 48),xlim = c(-60, -47))
...
for(i in 1:length(file_names)){
  ...
  LSfiles[[i]]$Vessel<-ifelse(substr(new_file_names[i],1,1)=="C","Calvert","Aqviq")
...
```{r}
ComIndex<-CommercialIndexData(datadir=datadir,bins=bins,add.portsampling=F,by.sex=F)
ComIndex$QUARTER<-quarter(ComIndex$DATE_TIME1)

 nafo3 = c( "3N", "3NA", "3NB", "3NC", "3ND", "3NE", "3NF", "3O", "3OA", "3OC", "3OD", "3OE", "3PN", "3PS", "3U")
 nafo4X = c( "4X", "4XL", "4XM", "4XN", "4XO", "4XP", "4XQ", "4XR", "4XS", "5YB", "5ZJ", "5ZM", "5ZN")
 nafo4VW = c("4VB", "4VC", "4VN", "4VS", "4W", "4WD", "4WE", "4WF", "4WG", "4WH", "4WJ", "4WK", "4WL", "4WM")
 
Fish_per_trip_nafo3 <- subset(ComIndex,YEAR%in%2024:2025&NAFAREA_ID%in%nafo3) |>
   group_by(TRIP,QUARTER)|>
  summarize(N_FISH=sum(EST_NUM_CAUGHT,na.rm=T))|>
  ungroup()|>
  group_by(QUARTER)|>
   summarize(N_FISH_AVG=mean(N_FISH,na.rm=T))

Fish_per_trip_nafo4X <- subset(ComIndex,YEAR%in%2024:2025&NAFAREA_ID%in%nafo4X) |>
   group_by(TRIP,QUARTER)|>
  summarize(N_FISH=sum(EST_NUM_CAUGHT,na.rm=T))|>
  ungroup()|>
  group_by(QUARTER)|>
...
Fish_per_trip_nafo4VW <- subset(ComIndex,YEAR%in%2024:2025&NAFAREA_ID%in%nafo4VW) |>
   group_by(TRIP,QUARTER)|>
  summarize(N_FISH=sum(EST_NUM_CAUGHT,na.rm=T))|>
  ungroup()|>
  group_by(QUARTER)|>
