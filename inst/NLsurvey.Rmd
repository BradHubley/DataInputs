...
As of 2026 - I have requested an update to this data and have received 2019-2026. Looks like 2019 data are somewhat different than shown in the last request. There was no survey in 2020. They changed vessels/gear same as Maritimes Region and no data post 2021 are reflective of that. They do not provide a correction factor between the new and old data for Atlantic halibut and recommend using the new years as a separate time series.

```{r}
NL_hal_LFraw <- read.csv(file.path(datadir,"NL_AtlanticHalibut_LF.csv")) #data from 2008-2019
...
```{r}
##I want to update the 2014-2019 part with the new data from 2019-2021, and then bring in the new time series of 2022-2025
NLstrat_length_abundance_detail_1996_2021 <- read.csv("C:/Users/harperd/Documents/Halibut/RDataVault/NLdata_received2025/NLstrat1_length_abundance_details_1996_2021.csv")
NLstrat_length_abundance_detail_2022_2025 <- read.csv("C:/Users/harperd/Documents/Halibut/RDataVault/NLdata_received2025/NLstrat1_length_abundance_details_2022_2025.csv")


NL_21_datUS <- NLstrat_length_abundance_detail_1996_2021 %>%
   filter(sex == "female+male+unsexed")

NL_datNEW <- NL_21_datUS %>%
   filter(between(survey.year, 2014, 2019))

NL_bindatNEW <- binNumAtLen(num.at.len = NL_datNEW$obs, len = NL_datNEW$length, bins = seq(0, 260, 5))

NL_bindatNEW <- as.data.frame(NL_bindatNEW)

NL_bindatNEW <- tibble::rownames_to_column(NL_bindatNEW, "length")

NL_bindatNEW.tab <- NL_bindatNEW %>%
   rename(count = NL_bindatNEW)
...
NL_25_datUS <- NLstrat_length_abundance_detail_2022_2025 %>%
   filter(sex == "female+male+unsexed")

NL_25_bindat <- binNumAtLen(num.at.len = NL_25_datUS$obs, len = NL_25_datUS$length, bins = seq(0, 260, 5))

NL_25_bindat <- as.data.frame(NL_25_bindat)

NL_25_bindat <- tibble::rownames_to_column(NL_25_bindat, "length")

NL_25_bindat.tab <- NL_25_bindat %>%
   rename(count = NL_25_bindat)

NL_25_bindat.tab$length<-gsub("L"," ", as.character(NL_25_bindat.tab$length))

NL_25_bindat.tab$length <- as.numeric(NL_25_bindat.tab$length)



##this is the end of adding new, and will incorporate below
...
NL_LFoverall <- NL_bindatNEW.tab %>% 
   ...
   left_join(NL_25_bindat.tab, by = "length") %>%
   rename("Yankee" = count.x.x, "Engel" = count.y.y, "Campelen1" = count.y, "Campelen2" = count.x, "NEW" = count)
...
C2<-  ggplot(NL_LFoverall) +
   geom_col(aes(length, Campelen2)) +
   xlim(0, 200)+
   labs(title = "3NOP RV Survey 2014-2021 Combined")+
   xlab("") + ylab("")+
   theme_classic()
...
New <- ggplot(NL_LFoverall) +
  geom_col(aes(length, NEW)) +
  xlim(0, 200) +
  labs(title = "3NOP RV Survey 2022-2025 Combined") +
  xlab("") + ylab("")+
  theme_classic()
...
NL_LFoverall_labels <- data.frame(gear = c("Yankee", "Engel", "Campelen1", "Campelen2", "New"), label = c("1973-1982", "1983-1991, 1994, 1995", "1996-2013", "2014-2021", "2022-2025"))

NL_LFoverall$gear <- factor(NL_LFoverall$gear, levels = c("Yankee", "Engel", "Campelen1", "Campelen2", "New"))

overall <- ggarrange(Y, E, C1, C2, New, ncol = 1, nrow = 5)

overall <- annotate_figure(overall, left = "Numbers at Length", bottom = "Length Bin Midpoint")

#french version
C2fr<-ggplot(NL_LFoverall) +
   geom_col(aes(length, Campelen2)) +
   xlim(0, 200)+
   labs(title = "Relevé par navire de recherche dans les divisions 3NOP, 
        2014-2019 combiné")+
   xlab("") + ylab("")+
   theme_classic()
   
C1fr<-ggplot(NL_LFoverall) +
   geom_col(aes(length, Campelen1)) +
   xlim(0, 200)+
   labs(title = "Relevé par navire de recherche dans les divisions 3NOP, 
        1996-2013 combiné")+
...
Efr<-ggplot(NL_LFoverall) +
   geom_col(aes(length, Engel)) +
   xlim(0, 200)+
   labs(title = "3NOP RV Survey 1983-1991, 1994, and 1995 Combined")+
   labs(title = "Relevé par navire de recherche dans les divisions 3NOP, 
        1983-1991, 1994, et 1995 combiné")+
   xlab("") + ylab("")+
   theme_classic()

overallfr <- ggarrange(Yfr, Efr, C1fr, C2fr, ncol = 1, nrow = 4)

overallfr <- annotate_figure(overallfr, left = "Nombres selon la longueur", bottom = "Point médian de la fourchette de longueur")
overallfr

png(paste0(file="C:/Users/harperd/Documents/Halibut/GitDataInputs/DataInputs/NL_LFfr.png"),
...
```
plotting mean number per tow?? - can't find the code for figure 25...maybe this could help? From 2014 assessment
```{r}
years<-1971:2013
ylim1<-c(0,0.4)

par(mfrow=c(1,1),omi=c(2,2,1,1),mar=c(2,2,2,.5), mgp=c(2,1,0), las=1)
plot(1971:1982,rv.dat[1:length(1971:1982),2],type='l',lty=1,xlim=c(1971,2013),ylim=ylim1,xlab='',ylab='',lwd=3)
lines(1983:1995,rv.dat[length(1971:1983):length(1971:1995),2],lwd=3)
lines(1996:2013,rv.dat[length(1971:1996):length(1971:2013),2],,lwd=3)
...
segments(1971, mean(rv.dat[1:length(1971:1982),2]), 1982, mean(rv.dat[1:length(1971:1982),2]), col="gray60", lwd=2)
segments(1983, mean(rv.dat[length(1971:1982):length(1971:1995),2]), 1995, mean(rv.dat[length(1971:1982):length(1971:1995),2]), col="gray60", lwd=2)
segments(1996, mean(rv.dat[length(1971:1995):length(1971:2012),2]), 2013, mean(rv.dat[length(1971:1995):length(1971:2012),2]), col="gray60", lwd=2)
text(1976,0.38, "Yankee")
text(1989,0.38, "Engel")
text(2004,0.38, "Campelen")

##this is a function from the 2014 framework for building that figure it seems
# get.3NOP.rv.dat<-function(start.year, end.year){
# require(RODBC)
# require(sqldf)
# require(reshape2)

dat<-read.csv("C:/Users/harperd/Documents/Halibut/GitDataInputs/DataInputs/data/NLHalibut_2014.csv", header = TRUE)
dim(dat)
#[1] 84366    23
names(dat)
dat<-dat[dat$SPEC==30&as.character(dat$NAFOZONE)%in%c('3N', '3O', '3P'),]
dat<-dat[as.character(dat$SEASON1)=='Spring'&dat$Month!=8,]
dat<-dat[dat$GEAR%in%c(1,2),]
dim(dat)
summary(dat)
sort(unique(dat$NAFOZONE))
with(dat, table(STDNO))
##mostly zeros, want to get expanded num

##remove any strata in a year with less than 2 tows
dat$yr_strat<-paste(dat$Year, dat$STRATUM)
x<-with(dat, table(yr_strat))
y<-x[x<2]
rm_strat<-names(y)
dat<-dat[!dat$yr_strat%in%rm_strat,]

##
dat<-dat[dat$Year>=start.year,]
...
dat$num[dat$Year<=1995]<-dat$STDNO[dat$Year<=1995]/stdsetarea[1]
dat$num[dat$Year>1995]<-dat$STDNO[dat$Year>1995]/stdsetarea[2]

library(sqldf)
dat2<-sqldf("SELECT Year, STRATUM, AVG(AreaNMsq) area, AVG(num) mean_no, Variance(num) var_no FROM dat GROUP BY Year, STRATUM")
dat3<-sqldf("SELECT Year, SUM(mean_no) tot_no, SUM(area) tot_area FROM dat2 GROUP BY Year")

with(dat3, plot(Year, tot_no/tot_area))

dat4<-merge(dat2, dat3, by.y='Year', by.x='Year', all.x=T)
names(dat4)
dat4$w_mean<-dat4$mean_no*dat4$area/dat4$tot_area
dat4$w_var<-dat4$var_no*(dat4$area/dat4$tot_area)^2

dat5<-sqldf("SELECT Year, SUM(w_mean) std_mean, sum(w_var) sum_var FROM dat4 GROUP BY Year")

# RV survey
rv.dat<-dat5
names(rv.dat)<-c('year','smean','smean.var')
rv.dat$smean[rv.dat$year<=1995]<-rv.dat$smean[rv.dat$year<=1995]*stdsetarea[1]   
rv.dat$smean[rv.dat$year>1995]<-rv.dat$smean[rv.dat$year>1995]*stdsetarea[2]   
rv.dat$smean.se[rv.dat$year<=1995]<-sqrt(rv.dat$smean.var[rv.dat$year<=1995])*stdsetarea[1] 
rv.dat$smean.se[rv.dat$year>1995]<-sqrt(rv.dat$smean.var[rv.dat$year>1995])*stdsetarea[2] 

lower<-rv.dat$smean-rv.dat$smean.se*2
upper<-rv.dat$smean+rv.dat$smean.se*2
...
NLu_binobs.tablong <- gather(NLu_binobs.tab, "year", "obs", -"length")
...
NL_25_strat_abundance_summary <-read.csv("C:/Users/harperd/Documents/Halibut/RDataVault/NLdata_received2025/NLstrat2_abundance_summary_2022_2025.csv")

ggplot(NL_25_strat_abundance_summary, aes(survey.year, mean)) +
   geom_line(size = 1.5, colour = "grey") +
   geom_errorbar(aes(ymin = mean.lcl, ymax = mean.ucl))+
   scale_y_continuous(breaks=seq(0,0.4,0.1)) +
   theme_classic()
