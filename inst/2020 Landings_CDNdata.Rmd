...
barplot(CanABM34p$Catch3BA~CanABM34p$Year,
 xlab= "Year", ylab="Canadian landing in Area 3 (t): 21B - 21A ",ylim=c(-400, 100))
dev.off()
...
barplot(CanABM34p$Catch4BA~CanABM34p$Year,
 xlab= "Year", ylab="Canadian landing in Area 4 (t): 21B - 21A ", ylim=c(-100, 100))
dev.off()
...
```{r comparison 21A & updated 21B}
  
# CHECK Line 206 comparison
# check differences between 21A and landCDN34   
CanA03 = get_21A(count="CDN", yearstart = ystart, datadir=datadir) %>%
    filter(Year >yCDN & Year<=yendCDN ) %>%
    filter(grepl("3",Division)) %>%
    group_by(Year) %>%
    summarise (CatchA03=sum(CatchA) )

CanA04 = get_21A(count="CDN", yearstart = ystart, datadir=datadir) %>%
...
CanB03 = CanB.final %>%
    filter(grepl("3",Division)) %>%  
    group_by(Year) %>%
    summarise (CatchB03=sum(Catch) )
CanB04 = mar04 %>%
    filter(grepl("4",Division)) %>%
    group_by(Year) %>%
    summarise (CatchB04=sum(Catch) ) 


CanCDN03 = merge(CanA03, CanB03)%>%
   mutate(Catch03D= CatchA03-CatchB03)
CanCDN04 = merge(CanA04, CanB04 )%>%
   mutate(Catch04D= CatchA04-CatchB04)  
CanCDN = merge (CanCDN03, CanCDN04)


############  below is copy from previous comparison. 
nafoABM34= cbind(nafoABM3,nafoABM4)
nafoABM34=nafoABM34[, -7] 
...
nafoABM34p = nafoABM34   %>%
    replace(is.na(.), 0)
tiff(file.path(wd,"figures","CDN_21A_B_3.tiff"), width=120, height=80, units="mm", pointsize = 10, bg="white", res=500)
barplot(nafoABM34p$Catch3BA~nafoABM34p$Year,
 xlab= "Year", ylab="Landing in Area 3 (t): 21B - 21A ",ylim=c(-400, 100))
dev.off()

tiff(file.path(wd,"figures","CDN_21A_B_4.tiff"), width=120, height=100, units="mm", pointsize = 10, bg="white", res=500)
barplot(nafoABM34p$Catch4BA~nafoABM34p$Year,
 xlab= "Year", ylab="Landing in Area 4 (t): 21B - 21A ", ylim=c(-100, 100))
dev.off()
