...
nafoA = get_21A(count="others", yearstart = ystart)  #1970
# sort(unique(nafoA$Division))

nafoB=get_21B(count="others",ystart,yend,1)
# unique(nafoB$Division)
nafoBdiv = get_21B(count="others",ystart,yend,2)
...
tiff("barplot_21B.tiff", width=120, height=100, units="mm", pointsize = 10, bg="white", res=500)
...
## Scal format of 21B  
nafoBg = get_21B(count="others",ystart,yendB,4)

# import scal data from 2014 assessment
scal=read.table("scal_catch.dat", header=F)
...
# get_data(db='marfis',data.dir=file.path(wd,'data'),fn.oracle.username = "hubleyb", fn.oracle.password = "R4#vmxtas", fn.oracle.dsn = 'ptran',usepkg='roracle',force.extract = T)
  get_data(db='marfis',data.dir=file.path(wd,'data'))
...
tiff("barplot_21A_B_3.tiff", width=120, height=80, units="mm", pointsize = 10, bg="white", res=500)
barplot(nafoABM34p$Catch3B_A~nafoABM34p$Year,
 xlab= "Year", ylab="Landing in Area 3 (t): 21B - 21A ",ylim=c(-400, 600))
dev.off()

tiff("barplot_21A_B_4.tiff", width=120, height=100, units="mm", pointsize = 10, bg="white", res=500)
...
nafoABM34p2= nafoABM34p    %>%
  filter (Year<=2000)
tiff("barplot_landingD_3_2000.tiff", width=120, height=90, units="mm", pointsize = 10, bg="white", res=500)
...
# landing NL 2016-2020, aggregate LL/OT/other
  landNL = read.csv("./NAFOlanding/landingNL.csv")
  ...
  landNL = landNL   %>%
    ...
    select(Year,Division, Gear,Catch1 ) %>%
    group_by(Year,Division, Gear) %>%
...
nafoCDN03 = get_21A(count="CDN", yearstart = ystart) %>%
    filter(Year >yendB & Year<=yend) %>%
    filter(grepl("3",Division)) %>%
    group_by(Year) %>%
    summarise (CatchA03=sum(CatchA) )

nafoCDN04 = get_21A(count="CDN", yearstart = ystart) %>%
    filter(Year >yendB & Year<=yend) %>%
    filter(grepl("4",Division)) %>%
    group_by(Year) %>%
...
nafoA3for = get_21A(count="Foreign", yearstart = ystart) %>%
    filter(Year >yendB & Year<=yend) %>%
    filter(grepl("3",Division))  

# foreign landing only in 4X
nafoA4for = get_21A(count="Foreign", yearstart = ystart)  %>%
    filter(Year >yendB & Year<=yend) %>%
    filter(grepl("4",Division)) 
...
### area 3 
tiff("barplot_LL3_up21B_SCAL.tiff", width=120, height=80, units="mm", pointsize = 10, bg="white", res=500)
barplot(nafoscalp$LL3d~nafoscalp$Year,
 xlab= "Year", ylab="LL3(t): previous SCAL - updated 21B",ylim=c(-100, 200))
dev.off()

tiff("barplot_OT3_up21B_SCAL.tiff", width=120, height=80, units="mm", pointsize = 10, bg="white", res=500)
barplot(nafoscalp$OT3d~nafoscalp$Year,
 xlab= "Year", ylab="OT3 (t): previous SCAL - updated 21B", ylim=c(-200, 200))
dev.off()

### area 4
tiff("barplot_LL4_up21B_SCAL.tiff", width=120, height=80, units="mm", pointsize = 10, bg="white", res=500)
barplot(nafoscalp$LL4d~nafoscalp$Year,
 xlab= "Year", ylab="LL4 (t): previous SCAL - updated 21B",ylim=c(-150, 150))
dev.off()

tiff("barplot_OT4_up21B_SCAL.tiff", width=120, height=80, units="mm", pointsize = 10, bg="white", res=500)
...
### area 3 
tiff("barplot_Bscal_LL3.tiff", width=120, height=80, units="mm", pointsize = 10, bg="white", res=500)
barplot(Bscalp$LL3d~Bscalp$Year,
 xlab= "Year", ylab="LL3 (t): previous SCAL - current 21B",ylim=c(-100, 200))  
dev.off()

tiff("barplot_Bscal_OT3.tiff", width=120, height=100, units="mm", pointsize = 10, bg="white", res=500)
barplot(Bscalp$OT3d~Bscalp$Year,
 xlab= "Year", ylab="OT3 (t): previous SCAL - current 21B", ylim=c(-200, 200))
dev.off()

### area 4
tiff("barplot_Bscal_LL4.tiff", width=120, height=80, units="mm", pointsize = 10, bg="white", res=500)
barplot(Bscalp$LL4d~Bscalp$Year,
 xlab= "Year", ylab="LL4 (t): previous SCAL - current 21B",ylim=c(-150, 150))
dev.off()

tiff("barplot_Bscal_OT4.tiff", width=120, height=80, units="mm", pointsize = 10, bg="white", res=500)
