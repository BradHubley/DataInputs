...
halsets <- halsets %>%
  left_join(ISSETPROFILE, by = "FISHSET_ID")
...
```{r}
sets_fs %>% count(YEAR)
#2009 entry station 274 has no set info...what to do with that point? (trip:100015694, set:100061377) NOT in ISSETPROFILE...why??
sets_sr %>% count(YEAR)
...
sets_ci %>% count(YEAR)
#2011 entry has no set info...(set:100160079, trip:100028073) not in ISSETPROFILE...why?
```
NAFO
```{r}
halsets %>% count(NAFO)
