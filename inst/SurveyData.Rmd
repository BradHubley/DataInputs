...
RandomSurvey<-RandomSurveyData(sp=30,datadir=datadir,add.gear=F,add.LF=T,bins=bins,by.sex=F,hook.data=T)
...
SpatialHub::bioMap(xlim = xl, ylim = yl, mapRes="MR", title = "Stratified Random Survey")
...
ComIndex$KGPHH<-ComIndex$EST_COMBINED_WT/ComIndex$NUM_HOOK_HAUL
CI<-with(ComIndex,tapply(KGPHH,YEAR, mean, na.rm=T))
plot(as.numeric(names(CI)),CI,xlab = '',ylim=c(0,0.5),type='b')

FixedSurvey$KGPHH<-FixedSurvey$EST_COMBINED_WT/FixedSurvey$NUM_HOOK_HAUL
FS<-with(FixedSurvey,tapply(KGPHH,YEAR, mean, na.rm=T))
lines(as.numeric(names(FS)),FS,type='b',col=2)

RandomSurvey$KGPHH<-RandomSurvey$EST_COMBINED_WT/RandomSurvey$NUM_HOOK_HAUL
RS<-with(RandomSurvey,tapply(KGPHH,YEAR, mean, na.rm=T))
lines(as.numeric(names(RS)),RS,type='b',col=3)
