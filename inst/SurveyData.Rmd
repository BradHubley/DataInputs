...
RandomSurveyRaph<-RandomSurveyData(sp=sp,datadir=datadir,by.sex=F,add.LF=F, hook.data=T)
...
RS<-RandomSurveyData(sp=sp,datadir=datadir,by.sex=F,add.LF=F, hook.data=F)
RS$STATION <- RandomSurveyRaph$ASSIGNED_STATION
RS$DEPTH[RandomSurveyRaph$DEPTH==0]<-NA

colNames1 <- c("FISHSET_ID","SET_NO","TRIP","DATE_TIME3","SOAKMINP3P1","DEPTH","LATITUDE","LONGITUDE","YEAR","STATION","STRATUM_ID","NAFAREA_ID","NUM_HOOK_HAUL","CFV","SPECCD_ID","total_other_species","total_target_species","broken_hook","empty_baited","empty_unbaited","other_species","target_species","missing_hook","total_sampled","avg_weight")

HLLRS[[i]] <- RandomSurveyRaph[,colNames1]
names(RandomSurveyRaph)
...
SpatialHub::bioMap(xlim = xl, ylim = yl, mapRes="MR", title = paste("Stratified Random Survey",yr),nafo=T)
