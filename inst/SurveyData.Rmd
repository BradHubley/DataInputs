...
# Update data from database 
 ...
 get_data(db='isdb',cxn=cxn, force.extract=T)
# open and check setprofile_wide to confirm that you have year, latitude & longitude
 ...
 Mar.utils::get_data_tables(schema="observer",cxn=cxn, data.dir = "C:/DFO-MPO/PESDData/MarDatawrangling", tables = c("ISFISHLENGTHS","ISSAMPLES"),force.extract=T)


Mar.utils::get_data_tables(schema="observer",cxn=cxn, data.dir = "C:/DFO-MPO/PESDData/MarDatawrangling", tables = c("ISFISHSETGEARGROUPS","ISGEARCOMPONENTS","ISGEARCOMPONENTSBAIT"))
Mar.utils::get_data_tables(schema="<NA>",cxn=cxn, data.dir = "C:/DFO-MPO/PESDData/MarDatawrangling", tables = c("ISFISHSETGEARGROUPS","ISGEARCOMPONENTS","ISGEARCOMPONENTSBAIT"),force.extract=T) 
...
```{r}
RandomSurveyLandmark<-RandomSurveyData(sp=30,datadir=datadir,by.sex=F,add.LF=F, hook.data=F)
...
RandomSurveySDM<-RandomSurveyData(sp=sp,datadir=datadir,by.sex=F,add.LF=F, hook.data=T)
RandomSurveySDM$avg_weight<-RandomSurveySDM$EST_COMBINED_WT / RandomSurveySDM$total_target_species
RandomSurveySDM$STATION <- RandomSurveySDM$ASSIGNED_STATION
RandomSurveySDM$DEPTH[RandomSurveySDM$DEPTH==0]<-NA
...
RandomSurvey <- RandomSurveySDM[,colNames1]
names(RandomSurveySDM)
...
subset(RandomSurvey1,abs(diff_count)>1&YEAR==2025)
subset(RandomSurvey1,abs(diff_count)>1&YEAR==2025&!is.na(L5))
subset(RandomSurvey2,abs(diff_count)>1&YEAR==2025)
...
write.csv(rs_w, "C:/Users/harperd/Documents/Halibut/GitDataInputs/randomsurvey_weightissues_2025.csv", row.names = FALSE)
...
write.csv(rs_l, "C:/Users/harperd/Documents/Halibut/GitDataInputs/randomsurvey_lengthissues_2025.csv", row.names = FALSE)
```
...
```{r}
yr=2024
RandomSurvey<-RandomSurveyData(sp=30,datadir=datadir,by.sex=F,add.LF=F, hook.data=F)
RS<-subset(RandomSurvey,YEAR==yr)
for(i in 1:nrow(RS)){
x1<-RS$LONG1[i]
y1<-RS$LAT1[i]
xl=c(x1-0.5,x1+0.5)
...
png(file.path(wd,'figures',paste0("StratfiedRandomSurveyMapSet",RS$SET_NO[i],yr,".png")), width =8, height = 6,units='in',pointsize=9, res=300,type='cairo')

                         
SpatialHub::bioMap(xlim = xl, ylim = yl, mapRes="UR", title = "Stratified Random Survey",nafo=T)
#points(LAT1~LONG1,subset(RandomSurvey,YEAR==yr),pch=21,bg=rgb(1,1,0,1))
#with(RS,segments(LONG1,LAT1,LONG3,LAT3,col='red'))
block <- read.csv(file.path(datadir,"Survey","newblock.csv"))
block_sf = st_as_sf(block, coords = c("xAEAm", "yAEAm"), crs = "ESRI:102001") #original data as a sf object

block_ll = st_transform(block_sf,crs = 4326)

blockLL = cbind(block[,c("PID","SID","POS")],st_coordinates(block_ll))
attr(blockLL,"projection") ="LL"
addLines(blockLL,col='grey')
with(RS,segments(LONG1,LAT1,LONG2,LAT2,col='red'))
points(LAT1~LONG1,RS,pch=21,bg=rgb(1,1,0,1))
points(LAT2~LONG2,RS,pch=21,bg=rgb(1,0,0,1))
#points(LAT3~LONG3,RS,pch=21,bg=rgb(0,1,0,1))
#ipoints(LAT4~LONG4,RS,pch=21,bg=rgb(0,0,1,1))
...
RVinputs<-prepRVdata(RVdata,bins=bins,years=1970:2025)
...
BarPlotLF(rvLF[1:30],yrs=Yrs1,rel=T,filen=file.path(wd,"figures","RVLF1"),rows=15,graphic='png',ax=c(rep(2,15),rep(4,15)),ylp=0.6,add.sample.size = F)

BarPlotLF(rvLF[31:length(rvLF)],yrs=Yrs2,rel=T,filen=file.path(wd,"figures","RVLF2"),rows=15,graphic='png',ax=c(rep(2,15),rep(4,15)),ylp=0.6,add.sample.size = F)

BarPlotLF(rvLF[1:30],yrs=Yrs1,rel=F,filen=file.path(wd,"figures","RVLF1_fr"),rows=15,graphic='png',ax=c(rep(2,15),rep(4,15)),ylp=0.6,xlab="Longueur (cm)",ylab="Nombre de flétans",add.sample.size = F)

BarPlotLF(rvLF[31:length(rvLF)],yrs=Yrs2,rel=F,filen=file.path(wd,"figures","RVLF2_fr"),rows=15,graphic='png',ax=c(rep(2,15),rep(4,15)),ylp=0.6,xlab="Longueur (cm)",ylab="Nombre de flétans",add.sample.size = F)
