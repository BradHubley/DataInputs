...
RandomSurveytest<-RandomSurveyData(sp=30,datadir=datadir,by.sex=F,add.LF=F)

RandomSurveyRaph<-RandomSurveyData(sp=30,datadir=datadir,by.sex=F,add.LF=F, hook.data=T)
...
colNames1 <- c("FISHSET_ID","SET_NO","TRIP","DATE_TIME3","SOAKMINP3P1","DEPTH","LATITUDE","LONGITUDE","YEAR","STATION","STRATUM_ID","NAFAREA_ID","NUM_HOOK_HAUL","total_other_species","total_target_species","broken_hook","empty_baited","empty_unbaited","other_species","target_species","missing_hook","total_sampled","avg_weight")

RandomSurvey <- RandomSurveyRaph[,colNames1]

FixedSurveyRaph<-FixedSurveyData(sp=30,datadir=datadir,by.sex=F,add.LF=F, hook.data=T)
...
colNames2 <- c("FISHSET_ID","SET_NO","TRIP","DATE_TIME3","SOAKMINP3P1","DEPTH","LATITUDE","LONGITUDE","YEAR","STATION","STRATUM_ID","NAFAREA_ID","NUM_HOOK_HAUL","total_other_species","total_target_species","avg_weight")
...
write.csv(rs_w, "C:/Users/harperd/Documents/Halibut/GitDataInputs/randomsurvey_weightissues.csv", row.names = FALSE)

#length from both datasets (using ISFISH and ISFISHLENGTHS)
rs_ISFISH_l <- subset(RandomSurvey2,abs(diff_count)>1)
rs_ISFISHLENGTHS_l <- subset(RandomSurvey1,abs(diff_count)>1)
rs_l <- rs_ISFISHLENGTHS_l %>% bind_rows(.,rs_ISFISH_l)
rs_l <- rs_l[!duplicated(rs_l$FISHSET_ID), ]

write.csv(rs_l, "C:/Users/harperd/Documents/Halibut/GitDataInputs/randomsurvey_lengthissues.csv", row.names = FALSE)
```
...
## plot all survey indices used in model
```{r}
#RVinputs$Index
png(file.path(wd,'figures',"Modelindices.png"), width =8, height = 8,units='in',pointsize=12, res=300,type='cairo')
par(mfcol=c(2,1), mar = c(0,4,0,0.5), omi = c(0.5, 0.25, 0.5, 0.25),las=1)

plot(NPT~year,RVinputs$Index, ylab="Number / tow", xlab = '', ylim=c(0,max(NPT)*1.2), type='b',pch=16,xlim=c(1970,2020),xaxt='n')
axis(1,lab=F)
points(2018,RVinputs$Index$NPT[RVinputs$Index$year==2018],col='red',cex=2)
plot(KgPKH~Year,subset(HSFixed_Index,Year<2021),type='b',pch=16,col='red',ylim=c(0,250),xlim=c(1970,2020), ylab="Kg / 1000 hooks")
