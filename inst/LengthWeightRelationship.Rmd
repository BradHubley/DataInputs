...
ISDB_halall <- ISDBdat %>%
  dplyr::select(-"FISH_ID", -"TRIP_ID", -"VESS_ID", -"LICENSE_NO", -"LANDING_DATE", -"FISHSET_ID", -"SETCD_ID", -"SPECSCD_ID.x",
...
#m5 and m8 are basically the same (according to the anova) which makes sense since NAFO seems to have bascially no effect (not significant and visualizations show total overlap)
```
```{r}
m1mf <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + QUARTER + SEXCD_ID + NAFO_ZONE + (1|YEAR) + (1|TRIPCD_ID), hal_datORmf)
...
plot_model(final_mod, type = "pred", terms = c("FISH_LENGTH", "QUARTER")) + theme_test()
