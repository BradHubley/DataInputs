...
n_occur <- data.frame(table(hal_gear$TRIP_ID))  %>% filter(Freq > 1) %>% rename(TRIP_ID = Var1)#47 trips where 2 types of gear are used
n_occur <- n_occur %>%
  select(TRIP_ID) 

n_occur2 <- n_occur[, "TRIP_ID"]
...
hal_gear <- hal_gear %>%
  ...
  mutate(GEARCD_ID = replace(GEARCD_ID, GEARCD_ID %in% c(41, 42), "gillnet")) %>%
...
Bring RV and ISDB together
```{r}
colnames(ISDB_halOR)
```
```{r}
colnames(ISDB_halall)
```
```{r}
colnames(MarRV)
```
