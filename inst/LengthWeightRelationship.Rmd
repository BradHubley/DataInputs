...
```{r}
hal_fish %>% 
  count(FISH_LENGTH) # 18,663 rows without fish length (of these, 2 have weights and the rest are missing both)

hal_rmNAwt <- hal_fish %>%
  drop_na(FISH_WEIGHT)#from 180,684 observations in hal_fish to 40,306 observations where weight is available
```

Trimming down the dataframe to useful columns
```{r}
hal_fishTrim <- hal_fish %>%
...
```
There are 19 fish where there is no corresponding NAFO (can likely just look it up using the coordinates?)
...
There are 19 fish that do not match up to a NAFO area, could use coordinates to match to a NAFO
...
```{r}
summary(hal_allrmNAwt$FISH_LENGTH)

summary(hal_allrmNAwt$FISH_WEIGHT)
```
...
```
This tells me there are data from the other fisheries/surveys that are outside our management unit. I need to remove those. And there are 9 fish where NAFO is not identified
