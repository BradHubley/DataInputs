...
```{r}
#get_data("rv", data.dir = "C:/Users/harperd/Documents/Halibut/RDataVault/", usepkg = "roracle",fn.oracle.username = "", fn.oracle.password = "", fn.oracle.dsn = "PTRAN", force.extract = T)
```

Extract MAR RV survey data from existing dump
```{r}
cxn <- ROracle::dbConnect(DBI::dbDriver("Oracle"), "harperd", "qkhxvh8b", "PTRAN")

get_data('rv', cxn=cxn, force.extract=FALSE)
...
setdata_halRV <- merge(hal, GSINF, by.x = c("MISSION","SETNO"), 
  ...
  dplyr::select("MISSION", "SETNO", "SPEC", "FSHNO", "FLEN", "FSEX", "FMAT", "FWT", "AGMAT", "AGE", "CLEN", "REMARKS.x", "SIZE_CLASS", "SDATE", "STRAT", "ELATITUDE", "ELONGITUDE", "ETIME", "REMARKS.y", "START_DEPTH", "END_DEPTH", "SURFACE_TEMPERATURE", "BOTTOM_TEMPERATURE", "BOTTOM_SALINITY", "STATION")
```
...
setdata_halRV <- setdata_halRV %>%
  mutate(yr_bin = 
           ...
           ifelse(YEAR %in% c(2016, 2017, 2018, 2019, 2020), "2016-2020", 
           ifelse(YEAR %in% c(2021, 2022, 2023, 2024, 2025), "2020-2025", "Error")))))))))))) #not yet included 2026...no summer survey done yet
...
Graphing the length by weight for NAFO and 5yr bins (when colouring the points by sex we lose even more data (from 1188 to 1204 fish not included in the figure below...))...maybe flattening out over time, but also likely because scales are free and range larger in recent years
```{r}
...
```{r}
#cxn <- ROracle::dbConnect(DBI::dbDriver("Oracle"), "harperd", "qkhxvh8b", "PTRAN")
get_data('isdb', cxn=cxn, force.extract = FALSE)
ISSETPROFILE_WIDE <- ISSETPROFILE_enwidener(ISSETPROFILE)


```

Pulling the halibut trips from the trips table (30 = halibut, 7057 = halibut longline survey, 7058 = halibut port sampling)  - BUT for 7058, there will be no weight in FISH_WEIGHT, it's held as gutted weight in the morphology table...so I'll handle those separately for this section and then work on combining them later
```{r}
hal_trips <- ISTRIPS %>%
...
hal_fishsets <- hal_trips %>%
  ...
  drop_na(FISHSET_ID) #need to drop NAs as there is 1 catches with NA for fish set in ISCATCHED that are being matched in the next join
```

Pulling out all the catches from the identified sets above, , and then filtering for just the halibut caught
```{r}
...
Pulling out all fish caught in the catches from the halibut trips
```{r}
...
```{r}
hal_fish %>% 
  count(FISH_LENGTH) # 11,819 rows without fish length (of these, 2 have weights and the rest are missing both)

hal_rmNAwt <- hal_fish %>%
  drop_na(FISH_WEIGHT)#from 157,160 observations in hal_fish to 40,302 observations where weight is available (very few new from last assessment)
```

Trimming down the dataframe to useful columns
```{r}
hal_fishTrim <- hal_rmNAwt %>%
  dplyr::select("TRIP_ID", "TRIP", "TRIPCD_ID", "VESS_ID", "LICENSE_NO", "BOARD_DATE", "LANDING_DATE", "FISHSET_ID", "SET_NO", "SETCD_ID", "SPECSCD_ID.x", "NAFAREA_ID", "CATCH_ID", "SPECCD_ID", "FISH_ID", "FISH_NO", "SEXCD_ID", "FISH_LENGTH", "FISH_WEIGHT", "MATCD_ID", "COMMENTS")
```
...
hal_fishTrim <- hal_fishTrim %>%
  mutate(yr_bin = 
           ...
           ifelse(YEAR %in% c(2011, 2012, 2013, 2014, 2015), "2011-2015",
           ifelse(YEAR %in% c(2016, 2017, 2018, 2019, 2020), "2016-2020",
           ifelse(YEAR %in% c(2021, 2022, 2023, 2024, 2025), "2021-2025", "Error"))))))))))))
...
Graphing the length by weight for NAFO and 5yr bins (when colouring the points by sex we lose even more data (from 1188 to 1204 fish not included in the figure below...)) - doesn't seem to be the case for re-run in 2026? says just removed 1 obs.
```{r}
...
Overall there are only 37 weighed fish from 2020, all on TRIPCD_ID 30 (not from survey). 2019 has only weighed fish on survey trips, and all seem to closely follow a log line (likely estimated). Seems like 2016 was the last time in a timeframe where lots of fish had weights and looks like it might have been measured? Additionally, there are 45 and 10 from 2022 and 2023 respectively...all from commercial fishery, and units are a bit of a mess
```{r}
...
hal_allrmNAwt <- hal_all %>%
  drop_na(FISH_WEIGHT) #goes from 272,023 observations to 55,197 when weight=NA is removed - maybe need to look at morph table for gutted?
...
hal_alltrips <- hal_alltrips %>%
  filter(TRIPCD_ID != "7058") #it's surprisingly few...
```
...
```{r}
hal_alltrips <- hal_alltrips %>%
  dplyr::select("TRIP_ID", "TRIP", "TRIPCD_ID", "VESS_ID", "LICENSE_NO", "BOARD_DATE", "LANDING_DATE", "FISHSET_ID", "SET_NO", "SETCD_ID", "SPECSCD_ID.x", "NAFAREA_ID", "CATCH_ID", "SPECCD_ID", "FISH_ID", "FISH_NO", "SEXCD_ID", "FISH_LENGTH", "FISH_WEIGHT", "MATCD_ID", "COMMENTS", "LAT1", "LONG1", "LAT2", "LONG2", "LAT3", "LONG3", "LAT4", "LONG4")
```
...
summary(hal_alltrips$FISH_WEIGHT) #clearly some unit inconsistencies
```
...
CHPhal <- hal_alltrips %>%
  ...
  dplyr::select(FISH_LENGTH, FISH_WEIGHT, BOARD_DATE, YEAR, TRIP)
summary(CHPhal)
...
WHhal <- hal_alltrips %>%
  filter(TRIPCD_ID == "12") %>%
  dplyr::select(FISH_LENGTH, FISH_WEIGHT, BOARD_DATE, YEAR)
summary(WHhal)
...
```
Looks like the only trip that might be an issue is redfish trip 90395.0 and it still could be real data...What was I basing that on??
```{r}
...
Sentinel <- allsurveyhal %>%
  ...
  dplyr::select(FISH_LENGTH, FISH_WEIGHT, BOARD_DATE)
...
snowcrab <- allsurveyhal %>%
  ...
  dplyr::select(FISH_LENGTH, FISH_WEIGHT, BOARD_DATE)
summary(snowcrab) #few data (n=15), weight appears to be g, length appears to be cm. range from 2015-2020. #No longer have access to this...but n was only 15 for last assessment...do we need it?
...
```
This tells me there are data from the other fisheries/surveys that are outside our management unit. I need to remove those. And there are 11 fish where NAFO is not identified (tripcd_ID for missing NAFO is "J09-0307", set #1, FISHSET_ID = 100159262...can pull coordinates for that)
```{r}
...
test_lenwt.dat.all<-test_hal[,c(1:8)] #why this step? It's already only 8 columns...

ISDB_outliers <- test_lenwt.dat.all %>% filter(out == "0") #here we lose 356 fish as outliers (not major at all in the overall dataset, it's 0.66%)
...
ISDB_halall <- hal_allNAFOfilter %>% #only reduces dataset by 12 fish, pretty good!
  drop_na(SEXCD_ID) %>%
  drop_na(FISH_LENGTH) %>%
  drop_na(FISH_WEIGHT) %>%
  drop_na(NAFO)

MarRV <- setdata_halRV %>% #reduces by 1213 fish, a lot more than ISDB set but still lots to work with i suppose?
  drop_na(FSEX) %>%
  drop_na(FLEN) %>%
  drop_na(FWT) %>%
  drop_na(NAFO)
...
MarRV <- MarRV %>%
  ...
  rename("TRIP" = "MISSION", "SEXCD_ID" = "FSEX", "SET_NO" = "SETNO", "SPECCD_ID" = "SPEC", "FISH_NO" = "FSHNO", 
         "FISH_LENGTH" = "FLEN", "FISH_WEIGHT" = "FWT", "COMMENTS" = "REMARKS", "BOARD_DATE" = "SDATE", "LAT2" = "ELATITUDE", "LONG2" = "ELONGITUDE", "NAFAREA_ID" = "NAME")
```
...
ISDB_halall <- ISDBdat %>%
  dplyr::select(-"FISH_ID", -"TRIP_ID", -"VESS_ID", -"LICENSE_NO", -"LANDING_DATE", -"FISHSET_ID", -"SETCD_ID", -"SPECSCD_ID.x", -"CATCH_ID", -"MATCD_ID", -"LAT1", -"LONG1", -"LAT3", -"LONG3", -"LAT4", -"LONG4")
```
...
#identify outliers (using a and b from the previous assessment log(a)=-5.021354 so a=0.00659559; b=3.124184)
mod_outliers<-nls(FISH_WEIGHT~a*FISH_LENGTH^b,data=test_halAll,start=c(a=0.00659559,b=3.124184),na.action=na.omit) #these results are the same with a=0.001 and b=3 as was used by nell in her getLW function from the previous assessment
...
all_outliers <- test_lenwt.dat.all %>% filter(out == "0") #here we lose 363 fish as outliers (the overall dataset, it's 0.60%) 
...
Now to see if I can include the shore-sampled Biological Sampling data. It's gutted weight, but try conversion factor of 1.14 which is used by C&P and Javitech as the conversion.
```{r}
biosamp <- ISTRIPS %>% filter(TRIPCD_ID == 7058)
biosamp <- biosamp %>% left_join(ISFISHSETS, by = "TRIP_ID") 
biosamp <- biosamp %>% left_join(ISCATCHES, by = "FISHSET_ID") %>% filter(SPECCD_ID == 30)
biosamp <- biosamp %>% left_join(ISSETPROFILE_WIDE, by = "FISHSET_ID")
biosamp <- biosamp %>% left_join(ISFISH, by = "CATCH_ID")
biosamp <- biosamp %>% left_join(ISFISHMORPHS, by = "FISH_ID") %>% filter(MRPHCD_ID %in% c(5,6,11,12,17,31,42,43,45)) #only 6 and 11 (gutted fresh, dressed)

#biosamp_mrphsummary <- biosamp %>% group_by(MRPHCD_ID) %>% summarize(n = n())
biosamp <- biosamp %>% mutate(NAFO = substr(NAFAREA_ID, 1,2))
#biosamp <- biosamp %>% select(TRIP, SPECCD_ID, FISH_ID, FISH_LENGTH, FISH_WEIGHT, MRPHCD_ID, QUANT_VALUE, YEAR, NAFO, SEXCD_ID) ##we have gutted weights up to 2025 from bio sampling

#Dressed = code 11 is all just in 1999...going to remove for the plot
biosamp <- biosamp %>% filter(MRPHCD_ID == 6)

biosamp %>% filter(YEAR >= 2000) %>% filter(SEXCD_ID == 2) %>%
ggplot() +
  geom_jitter(aes(FISH_LENGTH, QUANT_VALUE, colour = YEAR, alpha = 0.25)) +
...
summary(m1)
#simple lm of L X W
m2 <- lm(log(FISH_WEIGHT) ~ log(FISH_LENGTH), hal_datOR)
#(FE: length; RE: year)
m3 <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + (1|YEAR), hal_datOR)
#(FE: length; RE: trip)
m4 <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + (1|TRIPCD_ID), hal_datOR)
#(FE:length; RE: trip, year)
m5 <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + (1|YEAR) + (1|TRIPCD_ID), hal_datOR)
#(FE: length, sex; RE: trip, year)
m6 <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + SEXCD_ID + (1|YEAR) + (1|TRIPCD_ID), hal_datOR)
#(FE: length, quarter; RE: trip, year)
m7 <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + QUARTER + (1|YEAR) + (1|TRIPCD_ID), hal_datOR)
#(FE: length, nafo; RE: trip, year)
m8 <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + NAFO_ZONE + (1|YEAR) + (1|TRIPCD_ID), hal_datOR)
#(FE: length, sex, quarter; RE: trip, year)
m9 <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + SEXCD_ID + QUARTER + (1|YEAR) + (1|TRIPCD_ID), hal_datOR)
#(FE: length, sex, nafo; RE: trip, year)
m10 <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + SEXCD_ID + NAFO_ZONE + (1|YEAR) + (1|TRIPCD_ID), hal_datOR)
#(FE: length, quarter, nafo; RE: trip, year)
m11 <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + QUARTER + NAFO_ZONE + (1|YEAR) + (1|TRIPCD_ID), hal_datOR)



anova(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11) 

plot_model(m1, type = "pred", terms = c("FISH_LENGTH", "QUARTER", "NAFO_ZONE", "SEXCD_ID"))
```
...
sextest4 <- ggpredict(m1mf, terms = c("FISH_LENGTH", "SEXCD_ID"), condition = c(QUARTER = 2, NAFO_ZONE = 3), type = "re") #since QUARTER and NAFO_ZONE are categorical I don't actually need to put these conditions in here because ggpredict() picks a reference level of each categorical predictor anyways

ggplot(sextest4, aes(x = x, y = predicted, colour = group)) +
  geom_line() +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), linetype=0, alpha = .15)+
  labs(
    colour = get_legend_title(sextest4),
    x = get_x_title(sextest4),
    y = get_y_title(sextest4),
    title = get_title(sextest4)
  )


plot(sextest4)
plot_model(m1mf, type = "pred", terms = c("FISH_LENGTH", "SEXCD_ID"))
```
...
```{r}
Q1 <- hal_datOR %>% filter(QUARTER == 1)
Q2 <- hal_datOR %>% filter(QUARTER == 2)
Q3 <- hal_datOR %>% filter(QUARTER == 3)
Q4 <- hal_datOR %>% filter(QUARTER == 4)

FQ1_mod <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + (1|YEAR) + (1|TRIPCD_ID), Q1)
summary(FQ1_mod)
FQ2_mod <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + (1|YEAR) + (1|TRIPCD_ID), Q2)
summary(FQ2_mod)
FQ3_mod <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + (1|YEAR) + (1|TRIPCD_ID), Q3)
summary(FQ3_mod)
FQ4_mod <- lmer(log(FISH_WEIGHT) ~ log(FISH_LENGTH) + (1|YEAR) + (1|TRIPCD_ID), Q4)
summary(FQ4_mod)

```

Hal LW data collected by observers by year (from directed and not) And then RV
```{r}
hal_datOR %>%  filter(TRIPCD_ID %in% c("30", "7057")) %>%
  ggplot(aes(YEAR, stat = "count", fill = TRIPCD_ID, col = TRIPCD_ID)) +
  geom_bar()

hal_datOR %>%  filter(TRIPCD_ID != "RV") %>%
  ggplot(aes(YEAR, stat = "count", fill = TRIPCD_ID, col = TRIPCD_ID)) +
  geom_bar()

hal_datOR %>% filter(TRIPCD_ID == "RV") %>%
  ggplot(aes(YEAR, stat = "count")) +
  geom_bar()
