...
fsar_plot_base <- function(in.df, Assessment.Year=2024, language = c("English","French")) {
  ...
  if (language == "French") {
    x.lab <- "Année"
    y.lab <- "Capture (t)"
    legend.text <- c("Canada","Étranger", "TAC")
  }
  ...
  if (language == "French") {
    y.lab <- "Biomasse (kt)"
    legend.text <- c("Relevés du Flétan (modélisé)", "confiance à 95%","Relevés du Flétan (projection)","enveloppe de projection à 90%","Relevés du Flétan (indice)","Moyenne de l'indice sur 3 ans", "PRS", "PRL")
  }
  ...
  if (language == "French") {
    y.lab <- "Mortalité (1/ans)"
    legend.text <- c("pêche", "confiance à 95%", "naturelle", "95% confiance", "1,2FRMS", "FRMS", "0,8FRMS")
  }
  ...
  if (language == "French") {
    y.lab <- "Abondance (millions)"
    legend.text <- c("relevés de recherche (modélisé)", "confiance à 95%", "relevés de recherche (indice)")
  }
