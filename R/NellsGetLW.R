...
get.lenwt.all<-function(start.year, end.year){
  ...
  ######### OBSERVER ##############
  ch1 <- odbcConnect("ptran", uid = "denheyer", pwd = "plb479a")
  my.query <- paste("
...
")
  temp1 <- sqlQuery(ch1, my.query, errors=TRUE)
  ...
  ########  DFO RV   ###############
  ch2 <- odbcConnect("Bank", uid = "denheyer", pwd = "plb479a")
...
order by year, fsex, flen")
  ...
  temp2 <- sqlQuery(ch2, my.query, errors=TRUE)
