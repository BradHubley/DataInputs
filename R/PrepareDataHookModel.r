...
PrepareDataHookModel <-function(sp=30,wd="C:/Users/hubleyb/Documents/Halibut/data",add.gear=F){
  ...
  hook_data <- hookData(wdir=wd, species=sp)
  ...
  get_data(db='isdb',data.dir=wd)

  # filter for halibut longline survey
  ISTRIPTYPECODES= ISTRIPTYPECODES[ISTRIPTYPECODES$TRIPCD_ID %in% c(7057,7058),]
  ...
  fish.avg.weight <- select(ISCATCHES,FISHSET_ID,CATCH_ID,SPECCD_ID,EST_NUM_CAUGHT,EST_COMBINED_WT) %>%
  ...
  ISSETTYPECODES= ISSETTYPECODES[ISSETTYPECODES$SETCD_ID == 5,]
  ...
  fish <- left_join(select(ISCATCHES,FISHSET_ID,CATCH_ID,SPECCD_ID,EST_NUM_CAUGHT,EST_COMBINED_WT),fish.avg.weight) %>%
  ...
  # get assigned strata
  StnStrt<-read.csv(file.path(wd,"HS_STATION_STRATA.csv"))
...
write.csv(HALIBUTSURVEY,file.path(wd,"HalibutSurveyHookData.csv"),row.names = F)
