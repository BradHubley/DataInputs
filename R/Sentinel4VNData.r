#' @export
Sentinel4VNData <-function(sp=30,datadir,add.gear=F,add.LF=T,bins=seq(5,260,5),by.sex=T, LF.from='ISFISHLENGTHS'){
  ...
  isdb$ISTRIPTYPECODES= isdb$ISTRIPTYPECODES[isdb$ISTRIPTYPECODES$TRIPCD_ID %in% c(7052),]
  ...
  isdb$ISSETPROFILE$DEPTH <- rowMeans(isdb$ISSETPROFILE[,c("DEP1","DEP2","DEP3","DEP4")],na.rm=T)
  sets <- left_join(isdb$ISSETPROFILE[,c("FISHSET_ID","SET_NO","DATE_TIME3","SOAKMINP3P1","DEPTH","LATITUDE","LONGITUDE","YEAR")], isdb$ISFISHSETS[,c("FISHSET_ID","TRIP_ID","SET_NO","SETCD_ID","STATION","STRATUM_ID","NAFAREA_ID","NUM_HOOK_HAUL","GEAR_ID")], by=c('FISHSET_ID','SET_NO'))
  ...
  HALIBUTSURVEY$EST_NUM_CAUGHT[is.na(HALIBUTSURVEY$EST_NUM_CAUGHT)]<-0
   ...
   write.csv(HALIBUTSURVEY,file.path(datadir,"Sentinel4VNData.csv"),row.names = F)
