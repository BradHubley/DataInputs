...
BarPlotLF<-function(LF,yrs=2010:2016,LFyrs=yrs,bins=seq(0,220,5),filen='LF',rows=length(yrs),graphic='pdf',xl,rel=T,ylp=0.1,ymax,LS=81,add.sample.size=T,recline=NULL,wd=8,ht=11,bx=F,xlab="Length (cm)",ylab="Number of Halibut",toplab=NULL,ax,...){
    ...
    if(missing(xl))xlm<-c(0,length(bins))
        ...
        axis(1,1:length(bins),labels=F,tcl=-0.3)
        axis(1,labels=F,tcl=-0.6)
        if(i>1)axis(3,labels=F,tcl=0)
        if(rel==F){
            if(!is.na(ax[i]))axis(ax[i], pretty(c(0,yl2/1.3)),las=1)
            axis(2, pretty(c(0,yl2/1.3)),labels=F)
            axis(4, pretty(c(0,yl2/1.3)),labels=F)
        }
        if(rel==T){
            if(!is.na(ax[i]))axis(ax[i],seq(0,max(colSums(LF[[i]])),l=6),labels=seq(0,100,20),las=1)
            axis(2,seq(0,max(colSums(LF[[i]])),l=6),labels=F,las=1)
            axis(4,seq(0,max(colSums(LF[[i]])),l=6),labels=F,las=1)
        }
        if(i==length(LF)||i==rows){
            axis(1,at=pretty(xlm),labels=pretty(xlm)*diff(bins)[1])
        }
