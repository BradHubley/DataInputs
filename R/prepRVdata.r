...
prepRVdata<-function(RVdata,years, bins, pal=TRUE){
  ...
  for (i in 1:length(years)){
    ...
    stratF.lst[[i]]<-aggregate(survF.lst[[i]][c(paste("bin",bins,sep=''),"TUNITS")], list(survF.lst[[i]]$STRAT), mean)
    F.lst[[i]]<-colSums(sweep(stratF.lst[[i]][paste("bin",bins,sep='')],1,FUN='*',stratF.lst[[i]]$TUNITS))
    # Males
    survM.lst[[i]][is.na(survM.lst[[i]])]<-0
    stratM.lst[[i]]<-aggregate(survM.lst[[i]][c(paste("bin",bins,sep=''),"TUNITS")], list(survM.lst[[i]]$STRAT), mean)
    M.lst[[i]]<-colSums(sweep(stratM.lst[[i]][paste("bin",bins,sep='')],1,FUN='*',stratM.lst[[i]]$TUNITS))
    # Combined
    surv.lst[[i]][is.na(surv.lst[[i]])]<-0
    strat.lst[[i]]<-aggregate(surv.lst[[i]][c(paste("bin",bins,sep=''),"TUNITS")], list(surv.lst[[i]]$STRAT), mean)
  ...
  list(Index=Index,RVcatlenF=RVcatlenF,RVcatlenM=RVcatlenM,RVcatlenC=RVcatlenC, bins)
}
